...
@section('title')
    Edit Jadwal Sosialisasi SMA/SMK
@endsection
@section('page-title')
    Edit Jadwal Sosialisasi SMA/SMK
@endsection
@section('body')
    ...
    @section('content')
        <div class="row">
            <div class="col-12">
                <form action="{{ route('jadwal-sosialisasi.update', $jadwal->id) }}" method="post"
                      enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Edit Jadwal Sosialisasi SMA/SMK</h4>
                        </div>
                        <div class="card-body">
                            @if ($errors->any())
                                ...
                                <div class="col-md-10">
                                    <select class="form-select" id="id_sekolah" name="id_sekolah" required>
                                        @foreach($sekolahs as $sekolah)
                                            <option value="{{ $sekolah->id }}" {{ $sekolah->id == $jadwal->id_sekolah ? 'selected' : '' }}>{{ $sekolah->nama_sekolah }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                        ...
                                        <h5 class="font-size-14 mb-4"><i class="mdi mdi-arrow-right text-primary me-1"></i>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input class="form-control" type="date" autocomplete="off" name="tanggal_sosialisasi"
                                                       id="tanggal_sosialisasi" placeholder="Isikan tanggal sosialisasi sekolah" value="{{ $jadwal->tanggal_sosialisasi }}" required>
                                            </div>
                                                ...
                                                <div class="col-md-6">
                                                    <div class="mb-3">
                                                        ...
                                                        <input class="form-control" type="time" autocomplete="off" name="jam_sosialisasi"
                                                               id="jam_sosialisasi" placeholder="Isikan jam sosialisasi sekolah" value="{{ $jadwal->jam_sosialisasi }}" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="mb-3">
                                                        ...
                                                        <div class="input-group">
                                                            <input class="form-control" type="number" autocomplete="off" name="lama_sesi"
                                                                   id="lama_sesi" placeholder="Isikan lama sesi" value="{{ $jadwal->lama_sesi }}"  required>
                                        ...
                                        <h5 class="font-size-14 mb-4"><i class="mdi mdi-arrow-right text-primary me-1"></i>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input class="form-control" type="text" autocomplete="off" name="tempat_sosialisasi" id="tempat_sosialisasi"
                                                       placeholder="Isikan tempat sosialisasi" value="{{ $jadwal->tempat_sosialisasi }}">
                                            </div>
                                            <div class="mb-3">
                                                <label for="jumlah_siswa" class="form-label">Jumlah Siswa</label>
                                                <div class="input-group">
                                                    <input class="form-control" type="number" autocomplete="off" name="jumlah_siswa" id="jumlah_siswa"
                                                           placeholder="Isikan Jumlah Siswa" value="{{ $jadwal->jumlah_siswa }}">
                                ...
                                <div class="col-lg-5">
                                    <div class="mt-4 mt-xl-0">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="checkbox" id="proyektor" name="proyektor" value="Disediakan" {{ $jadwal->proyektor == 'Disediakan' ? 'checked' : '' }}>
                                            <label class="form-check-label" for="proyektor">Proyektor</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="checkbox" id="speaker" name="speaker" value="Disediakan" {{ $jadwal->speaker == 'Disediakan' ? 'checked' : '' }}>
...
{{--                                                        <option value="Persiapan Sosialisasi">Persiapan Sosialisasi</option>--}}
                                                        @foreach(["Selesai" => "Selesai", "Batal" => "Batal", "Menunggu Jadwal" => "Menunggu Jadwal", "Persiapan Sosialisasi" => "Persiapan Sosialisasi"] AS $status_sosialisasi => $status_sosialisasi_label)
                                                            <option value="{{ $status_sosialisasi }}" {{ old("status_sosialisasi", $jadwal->status_sosialisasi) == $status_sosialisasi ? "selected" : "" }}>{{ $status_sosialisasi_label }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
...
{{--                                                        <option value="Sudah Dikonfirmasi">Sudah Dikonfirmasi</option>--}}
                                                        @foreach(["Dibalas" => "Dibalas", "Hanya Dibaca" => "Hanya Dibaca", "Sudah Dikonfirmasi" => "Sudah Dikonfirmasi"] AS $status_follup => $status_follup_label)
                                                            <option value="{{ $status_follup }}" {{ old("status_follup", $jadwal->status_follup) == $status_follup ? "selected" : "" }}>{{ $status_follup_label }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-md-11">
                                    <textarea class="form-control" type="number" autocomplete="off" name="note" id="note">{{ $jadwal->note }}</textarea>
                                </div>
                            </div>
